...
for(ns=1:numel(InputStruct))
    ...
    if strcmpi(PartToDelete,'Analysis') % special case
        ...
        unix(sprintf('rm -rf %s', fullfile( outpath,InputStruct_temp.PREFIX,'perf_proc_p2',sprintf('pipe_%s',pipe_idx_todel.P2{1}),ParamStruct_aug.Variable_ID )) );
    else
        ...
        if ~isempty(cell_idx_todel.P2)
            for i=1:numel(pipe_idx_todel.P2)
                unix(sprintf('rm -rf %s', fullfile( outpath,InputStruct_temp.PREFIX,'perf_proc_p2',sprintf('pipe_%s',pipe_idx_todel.P2{i}) )) );
            end
        end
        % then delete all specified p1 pipelines
        if ~isempty(cell_idx_todel.P1)
            for i=1:numel(pipe_idx_todel.P1)
                unix(sprintf('rm -rf %s', fullfile( outpath,InputStruct_temp.PREFIX,'perf_proc_p1',sprintf('subpipe_%03u',pipe_idx_todel.P1{i}) )) ); % _seg gets deleted too
            end
        end
        ...
        if ~isempty(cell_idx_todel.Seg)
            for i=1:numel(pipe_idx_todel.Seg)
                ...
                e = dir(sprintf('%s/%s/perf_proc_p1/subpipe_*',outpath,InputStruct_temp.PREFIX));
                for j=1:numel(e)
