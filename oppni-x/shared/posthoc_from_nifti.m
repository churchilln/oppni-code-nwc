function out = posthoc_from_nifti( INPUT_VOLS, MASK, volno, design_mat, analysis_model, model_contrast, THRESH_METHOD, out_folder_name, censor, ROIvol )
%
% Input:
%
%         INPUT_VOLS : Either (1) a cell array of (strings) giving nifti file location/name corresponding to individual participants or (2) 
%                      a (string) giving the name of a single 4D file containing all participants' data
%               MASK : string, specifying location/name of binary brain mask
%
...
if iscell(INPUT_VOLS)
    disp('assuming input volumes are provided as a list')
    if numel(INPUT_VOLS) ~= ns
        error('number of input volumes do not match rows of design matrix!');
    end
    for i=1:numel(INPUT_VOLS)
        V=load_untouch_niiz(INPUT_VOLS{i});
        tmp=nifti_to_mat(V,MB);
        datamat(:,i) = tmp(:,volno);
    end
else
    V=load_untouch_niiz(INPUT_VOLS);
    datamat=nifti_to_mat(V,MB);    
end

if isempty(censor) censor = ones(size(datamat,2),1); end 
param_type='image';
%%---
