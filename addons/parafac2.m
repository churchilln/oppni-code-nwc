...
%
%
% A = spatial mode: (vox x K)
% C = diagonals of Dk, stacked (subj x K)
% H = F mode matrix? (KxK)... F=H'*H;
% P = K-length cell containing (time x K) timeseries
...
if Options(4)==1
   ...
   for f = 1:F
      ...
      for s = 1:splits
         ...
         [a,h,c,p]=parafac2_parallel(Xmiss,f,Constraints,Opt);
...
if nargin<5
   if initi==0
      ...
      Opt(4) = 0;
      Opt(5) = 1;
      [A,H,C,P,bestfit]=parafac2_parallel(X,F,Constraints,Opt);
      AllFit = bestfit;
      for i = 2:NumRep
         Opt(3) = 2;   % Init with random
         [a,h,c,p,fit]=parafac2_parallel(X,F,Constraints,Opt);
         AllFit = [AllFit fit];
         if fit<bestfit
            A=a;H=h;C=c;P=p;
            bestfit = fit;
   ...
   elseif initi==1
      ...
      Aset = [];
      for k = 1:K
        [Atmp sigos] = svd( X{k},'econ' );
        Aset = [Aset Atmp(:,1: min([size(X{k},2),F]) )];
      end
      [A sigos] = svd( Aset,'econ' ); A=A(:,1:F);
...
%if initi~=1 %% MODIFY: removed because SVD doesn't calc. this anymore!
   XtX=X{1}'*X{1}; % Calculate for evaluating fit (but if initi = 1 it has been calculated) MODIFY: originally X{1}*X{1}'
   for k = 2:K
      XtX = XtX + X{k}'*X{k}; %% MODIFY: originally X{k}*X{k}'
   end
%end  
fit    = sum(diag(XtX));
oldfit = fit*2;
