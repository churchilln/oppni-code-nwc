...
% rsynthond1.m
%
% function [approx,tapprox,napprox]=rsynthond1(regu,wsf,nbvoies);
%-------------------------------------------------
...
npb=0;
nph=0;
g=(-1).^(nph:nph-1+max(size(h))).*fliplr(h);
hh=h;
clear h
...
if test==1,
   ...
   for k=1:1:nbvoies-1
      fprintf('%d  ',k)
      [b,nb]=upsample(b,nb);
      [approx,napprox]=convoluteur(approx,napprox,b,nb);
   end
end
...
if test==2,
   ...
   napprox=nph;
   for k=1:1:nbvoies-1
      [approx,napprox]=upsample(approx,napprox);
   end
   b=h2;
   nb=npb;
   for k=1:1:nbvoies-1
...
%approx=approx*2^(-nbvoies/2);
tapprox=0:1:max(size(approx))-1;

tapprox=tapprox+napprox;
tapprox=tapprox*2^(-nbvoies);
%plot(tapprox,approx)
