...
UPDATE RELEASE 03/2014:
The RICE wavelet toolbox is no longer used for 2D analysis (images). The code is entirely MATLAB and stand-alone. Also, image size can be arbitrary and is not constrained to be a power of 2 any longer.
...
In order to run the demo files ("demo_MF_BS_tool.m" and "demo_MF_BS_test_constancy.m"), you will need to download the example data available on my web site. Alternatively, you can of course use your own data (modify ll. 49 in the scripts).
...
USAGE OF "MF_BS_test_constancy.m"
- Input parameters, output and usage are commented in detail in "demo_MF_BS_test_constancy.m".
- The method is based on double bootstrap. You have to use a reasonable number of primary and double bootstrap resamples for relevant results (practically starting from a number of the order of 50).
- The most critical parameter is the number M of time windows used. Practically, it controls the trade-off between power of the test, and time resolution.
