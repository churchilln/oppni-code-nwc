function [confidence]=BS_CI(estimates, paramBS, Alpha);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%       paramBS  = struct('Nresamp1', 999, 'Nresamp2', 50, 'blocklength', 1, 'Method', [3 4 5]);
%       Alpha=0.05;
%   % NOTE: corresponding BS estimates must exist, e.g. generated by function BSsample
%       [confidence] = BS_CI(estimates, paramBS, Alpha);
%
...
% Check input arguments: paramBS, Alpha
...
Method=paramBS.Method; CIdisp=[];
if isempty(Method); return; end;
if find(Method==1);                 NOR=1; CIdisp=[CIdisp 1];       else NOR=0; end
if find(Method==2);                 BAS=1; CIdisp=[CIdisp 2];       else BAS=0; end
if find(Method==3);                 PER=1; CIdisp=[CIdisp 3];       else PER=0; end
if find(Method==4);                 STU=1; CIdisp=[CIdisp 4];       else STU=0; end
if find(Method==5)|find(Method==6); ADJ=1;                          else ADJ=0; end
if find(Method==5);                 BASADJ=1; CIdisp=[CIdisp 5];    else BASADJ=0; end
if find(Method==6);                 PERADJ=1; CIdisp=[CIdisp 6];    else PERADJ=0; end
...
% stdT
if STU    
    try  NB=isreal(estimates.stdT); catch error('Estimate estimates.stdT must be a vector of size paramBS.Nresamp1'); end
...
% TT
if BASADJ|PERADJ
    try  NB=isreal(estimates.TT); catch error('estimates.TT must be a matrix of size paramBS.Nresamp1 x paramBS.Nresamp2.'); end
