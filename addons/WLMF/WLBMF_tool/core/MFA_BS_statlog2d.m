function [logstatDyad, parBSest] = MFA_BS_statlog2d(data, methodWT, paramEST, paramBS, Jflag, T_S);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function [logstatDyad, parBSest] = MFA_BS_statlog2d(data, methodWT, paramEST, paramBS, Jflag, T_S);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[coef, leaders, nj] = DxLx2d(data, MomNul, gamint, sym);
% [coef, leaders, nj] = DxLx2d(data, MomNul, gamint);
if DWT; nbvoies=length(nj.W); end
if LWT; nbvoies=length(nj.L); end
if DWT&LWT; nbvoies=min(length(nj.W),length(nj.L)); end;
scale=2.^(1:nbvoies);

for j=1:nbvoies
    supcoef(j)=coef(j).supcoef;
    supcoefnoint(j)=coef(j).supcoefnoint;
    if ~isempty(coef(j).supcoefL)
        supcoefL(j)=coef(j).supcoefL;mincoef(j)=coef(j).mincoef;
    else
        supcoefL(j)=NaN;mincoef(j)=NaN;
    end
end
...
if T_S
    ...
    if DWT
        ...
        [estimatesW, LEstW]=resample2d_T_S(coef, nj.W, paramBS, paramEst, J1BS);
    ...
    if LWT
        ...
        [estimatesL, LEstL]=resample2d_T_S(leaders, nj.L, paramBS, paramEst, J1BS);
        % [estimatesW{j}, LEstW{j}]=ndimBSresample(AbsdqkW, parBSest, paramEst, TTsave);
        if J1BS>1; tmpjflag=1; else tmpjflag=0; end
        for j=1:nbvoies
            if j==min(j1Z, j1C); tmpjflag=0; end
    ...
    for j=1:nbvoies
        ...
        %-- 2c: Coefficients - Estimates and Bootstrap
        if DWT
            [estimatesW{j}, LEstW{j}]=resample2d(coef(j), parBSest, paramEst);
            %[estimatesW{j}, LEstW{j}]=ndimBSresample(coef(j).value, parBSest, paramEst, TTsave);
            ESTW(:,j)=estimatesW{j}.t;      % read out estimates at this scale j
            % read out Bootstrap Variance Estimates and Bootstrap Resamples at this scale
            if doB1&~Jflag
                ESTBW(:,:,j)=estimatesW{j}.T;
        ...
        %-- 2d: Leaders- Estimates and Bootstrap
        if LWT
            [estimatesL{j}, LEstL{j}]=resample2d(leaders(j), parBSest, paramEst);
            %[estimatesL{j}, LEstL{j}]=ndimBSresample(leaders(j).value, parBSest, paramEst, TTsave);
            ESTL(:,j) =estimatesL{j}.t; % read out estimates at this scale j
            % read out Bootstrap Variance Estimates and Bootstrap Resamples at this scale
            if doB1&~Jflag
                ESTBL(:,:,j)=estimatesL{j}.T;
