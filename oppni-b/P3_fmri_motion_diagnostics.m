...
if ~exist(  fullfile(outpath,'_group_level','QC','qc.quant',['QCStruct_quant_pipe_',PipeStruct_aug.PNAME{1},'.mat'])  ,'file')
    ...
    % func
    Mref_func = load_untouch_niiz([outpath,'/_group_level/masks/pipe_',PipeStruct_aug.PNAME{1},'/func_brain_mask_grp.nii']);
    V    = load_untouch_niiz([outpath,'/_group_level/brain_maps/pipe_',PipeStruct_aug.PNAME{1},'/func_tAV_grp.nii']);
    avec = nifti_to_mat(V,Mref_func);
    V    = load_untouch_niiz([outpath,'/_group_level/brain_maps/pipe_',PipeStruct_aug.PNAME{1},'/func_tSD_grp.nii']);
    ...
    for ni=1:numel(qc_subj_idxes)
        ...
        QCStruct_quant(ni).N_func = InputStruct_ssa.N_func;
        ...
        for nr=1:InputStruct_ssa.N_func
            ...
            QCStruct_quant(ni).frun(nr).mot_avg_tot = mean(rmsd);
            QCStruct_quant(ni).frun(nr).mot_max_tot = max(rmsd);
            % relative [motion] framewise disp
            rmsd = mean( (mpe(2:end,:)-mpe(1:end-1,:)).^2,2);
            QCStruct_quant(ni).frun(nr).mot_avg_rel = mean(rmsd);
            QCStruct_quant(ni).frun(nr).mot_max_rel = max(rmsd);
    
            % unwarped mask [mask quality / brain shape] - total brain volume
            M=load_untouch_niiz(sprintf('%s/warp/func%u_mask.nii.gz',opath2f,nr));
            QCStruct_quant(ni).frun(nr).maskvol_unwarp = sum(M.img(:));
            % warped mask [mask quality / brain shape] - total brain volume
            M=load_untouch_niiz(sprintf('%s/postwarp/func%u_warped_mask.nii.gz',opath2f,nr));
            QCStruct_quant(ni).frun(nr).maskvol_warp = sum(M.img(:));
            % jaccard overlap viz groupmask
            a=Mref_func.img(:);
            b=M.img(:);
            QCStruct_quant(ni).frun(nr).mask_ovl_grp = sum( a.*b ) ./ sum( double(a | b) );
    
            V = load_untouch_niiz(sprintf('%s/postwarp/func%u_warped_tav.nii.gz',opath2f,nr));
            QCStruct_quant(ni).frun(nr).corr_tav = corr( avec, nifti_to_mat(V,Mref_func));
            V = load_untouch_niiz(sprintf('%s/postwarp/func%u_warped_tsd.nii.gz',opath2f,nr));
            QCStruct_quant(ni).frun(nr).corr_tsd = corr( svec, nifti_to_mat(V,Mref_func));
    
            V = load_untouch_niiz([opath4f,'/func',num2str(nr),'_fullproc.nii.gz']);
            volmat = nifti_to_mat(V,Mref_func);
            ...
            QCStruct_quant(ni).frun(nr).dvr_avg_tot = mean(rmsd);
            QCStruct_quant(ni).frun(nr).dvr_max_tot = max(rmsd);
            % dvars [motion] framewise disp
            rmsd = mean( (volmat(:,2:end)-volmat(:,1:end-1)).^2,1);
            QCStruct_quant(ni).frun(nr).dvr_avg_rel = mean(rmsd);
            QCStruct_quant(ni).frun(nr).dvr_max_rel = max(rmsd);
            % signal scaling
            QCStruct_quant(ni).frun(nr).tav_map_av = mean( mean(volmat,2) );
            QCStruct_quant(ni).frun(nr).tav_map_sd =  std( mean(volmat,2) );
            QCStruct_quant(ni).frun(nr).tsd_map_av = mean( std(volmat,0,2) );
            QCStruct_quant(ni).frun(nr).tsd_map_sd =  std( std(volmat,0,2) );
            % tsnr [amt of noise] average and variability
            QCStruct_quant(ni).frun(nr).tsnr_av = mean(  mean(volmat,2)./(std(volmat,0,2)+eps) );
            QCStruct_quant(ni).frun(nr).tsnr_sd =  std(  mean(volmat,2)./(std(volmat,0,2)+eps) );
            % kurtosis [presence of outliers]
            krt = kurtosis( volmat,0,2 );
            thr_krt = prctile( kurtosis( randn(size(volmat,2),5000) ), 99.5 );
            QCStruct_quant(ni).frun(nr).krt_peak = prctile(krt,99.5);
            QCStruct_quant(ni).frun(nr).krt_avg  = mean(krt);
    ...
    %     for nr=1:InputStruct_ssa.N_func
    %         qcmatm(ni,1) = QCStruct_quant(ni).frun(nr).mot_avg_tot;
    %         qcmatm(ni,2) = QCStruct_quant(ni).frun(nr).mot_max_tot;
    %         qcmatm(ni,3) = QCStruct_quant(ni).frun(nr).mot_avg_rel;
    %         qcmatm(ni,4) = QCStruct_quant(ni).frun(nr).mot_max_rel;
    %     end
    % end
    ...
    % for ni=1:numel(subject_list)
    %     for nr=1:InputStruct_ssa.N_func
    %         qcmatd(ni,1) = QCStruct_quant(ni).frun(nr).dvr_avg_tot;
    %         qcmatd(ni,2) = QCStruct_quant(ni).frun(nr).dvr_max_tot;
    %         qcmatd(ni,3) = QCStruct_quant(ni).frun(nr).dvr_avg_rel;
    %         qcmatd(ni,4) = QCStruct_quant(ni).frun(nr).dvr_max_rel;
    %     end
    % end
...
for ni=1:numel(QCStruct_quant)
    for nr=1:QCStruct_quant(ni).N_func
        kq=kq+1;
        xtmp   = QCStruct_quant(ni).frun(nr);
...
filo = fopen( fullfile(outpath,'_group_level','QC','qc.quant',['table_func_stats_pipe_',PipeStruct_aug.PNAME{1},'.txt']),'w');
