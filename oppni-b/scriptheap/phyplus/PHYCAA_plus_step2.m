...
for(pcs=pcsets)
    ...
    for(r=1:Nruns) 
        ...
        tset = [a;b;c];
        tset = bsxfun(@minus, tset, mean(tset,1)); % subtract mean
        tset = bsxfun(@rdivide, tset, sqrt(sum(tset.^2))); % unit-norm
        ...
        if( ~isempty( Tset ) )
            ...
            % (B^2=estimated var) / (residX^2=total variance)
            Vset       = bsxfun(@rdivide, Bset.^2, sum(residualMat{r}.^2,2));
            % catch for zero-var
            Vset(~isfinite(Vset))=0;
            ...
            vmed_nonneur=median( Vset( dataInfo.physio_map < ( 0.50 - (dataInfo.comp_crit/2) ), : ) );
            vmed_neuronl=median( Vset( dataInfo.physio_map > ( 0.50 + (dataInfo.comp_crit/2) ), : ) );
