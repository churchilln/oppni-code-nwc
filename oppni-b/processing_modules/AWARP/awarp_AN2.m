...
% .A MORE EFFICIENT WAY TO RUN ANTS COMPARED TO V1
...
if ~exist( sprintf('%s/anat_warped.nii.gz',odir) ,'file')
    ...
    elseif contains(Adataset,'.nii')
        ...
        error('unrecognized Adataset format');
    end
    ...
    % unix(sprintf(['antsRegistration ', ... %% --> deprec, changed for v5+ compatbility
    % '-d 3 ', ...
    % '-r [ %s, %s , 1 ] ', ...
...
unix(sprintf(['antsRegistration ',...
    ...
    '--metric Mattes["%s","%s",1,32,Regular,0.3] ', ...
    '--convergence "[10000x5000x2000,1e-8,20]" ', ...
    ...
    '--shrink-factors 4x2x1 ', ...
    '--smoothing-sigmas 4x2x1vox ', ...
    '--transform SyN[0.20,3,0] ', ...
    '--metric Mattes["%s","%s",0.5,32] ', ...
    '--metric CC["%s","%s",0.5,4] ', ...
    '--convergence "[100x100x50,1e-6,5]" ', ...
    '--shrink-factors 4x2x1 ', ...
    '--smoothing-sigmas 1x0.5x0vox ',...
    '--output "[%s,%s_diff.nii.gz,%s_inv.nii.gz]" '], ...
    fixd, movn,fixd, movn,fixd, movn,fixd, movn,fixd, movn,fixd, movn, nomen,nomen,nomen));
