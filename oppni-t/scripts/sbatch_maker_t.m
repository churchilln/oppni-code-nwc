close all;
clear;
disp('go onn'); % batching for optimized BW value - KKDE-truncated (more time intensive!!)
...
for i=1:numel(e)
    if ~strcmpi(e(i).name,'sub-NCOV1F006_ses-V01')
        ...
        fido = fopen(sprintf('super_slurm_%u_optim_t.sh',i),'w');
        vcount = 0;
        for vb=1:VBMAX
            ...
            filename = sprintf('tex_sbatch_s%u_%u_optim_t.sl',i,vb);
            ...
            % write job command
            fprintf(fid,'matlab -nodisplay -nojvm -singleCompThread -r "addpath /lustre03/project/6071833/nwc/NIFTI_tools; addpath /lustre03/project/6071833/nwc/tex_code; texmap_wrapper3( ''%s'', ''%s'', [%s], 0, ''%s_vbatch%uof%u'', [], [], [], [], ''KDE-GT'', ''optim'' ); exit;"\n',...
        ...
        if vcount==0
            ...
            unix(sprintf('rm super_slurm_%u_optim_t.sh',i))
        else
