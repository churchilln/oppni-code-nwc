...
for i=1:numel(e) %list1(WM):  RAD=5,B=0.175 / RAD=7,B=0.10 / RAD=9,B=0.070 / RAD=11, B=0.055
    if ~strcmpi(e(i).name,'sub-NCOV1F006_ses-V01')
        ...
        outfile = sprintf('texturial_matfiles_moar2/%s_tm',prfix);

        texmap_wrapper3( t1_file, mskfile, [10 49], 1, outfile, 7, 7, [], [], 'HDE', 'estim',0,1 ); %[0.0425]
    else
        disp('not on list!');
    end
end
...
for i=1%:numel(e) %list1(WM):  RAD=5,B=0.175 / RAD=7,B=0.10 / RAD=9,B=0.070 / RAD=11, B=0.055
    if ~strcmpi(e(i).name,'sub-NCOV1F006_ses-V01')
        ...
        texmap_wrapper3( t1_file, mskfile, list1, 1, outfile, 7, 7, [], [], 'HDE', [0.10],0,1 ); %[0.0425]
    else
        disp('not on list!');
    end
end
